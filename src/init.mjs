...
import inject_player_gold_in_scene_nav from "./scene_nav_button.mjs";
...
Hooks.once("ready", () => {
  if (
    game.user.isGM &&
    !window.pr.api.get("first-time-startup-notification-shown")
  )
...
function first_time_startup_notification() {
  ...
  notification.find("a.guide-me").on("click", (e) => {
    ...
    setTimeout(() => {
      window.pr.notifications.clear(notification);
    }, 2000);
  });
